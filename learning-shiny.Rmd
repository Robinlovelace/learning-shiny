...
any real (or fictitious) quantifiable problem you can think of. It's a cliche
worth repeating: with **shiny**, you really are limited only by your imagination.

Because **Shiny** is an open source software
project with a very strong [community](https://groups.google.com/d/forum/shiny-discuss), it has excellent documentation,
including a user group, a Gallery of examples and (most recommended of all)
a fantastic up-to-date [online tutorial](http://shiny.rstudio.com/tutorial/) maintained by
[RStudio](http://www.rstudio.com/). Unless you're an R user [who lives under a rock], 
you'll be aware of RStudio which has revolutionised R's user appeal with a
slick user interface and dozens of tools that make life easier for R
beginners and [developers](http://shiny.rstudio.com/articles/) alike. RStudio is a profit making company
that has done more than any other organisation to promote, develop and
communicate R to the new generation. To make a (perhaps unfair) analogy,
RStudio is to R what Canonical is/was to Linux: a commercial enterprise
that greatly improved the user-friendliness of the underlying program.

RStudio's impact on R is hard to overstate and it's employees are
...
This tutorial was even written and compiled in RStudio, with its
seamless integration with [knitr](http://yihui.name/knitr/) and
[RMarkdown](http://rmarkdown.rstudio.com/) which is now my default
...
RMarkdown to save all the `\`s needed for pure Late X!]

To cut a long story short, I RStudio for this tutorial:
it even provides a button to test your applications with a single 
click^[RStudio seemingly does **not** provide keyboard shortcuts for Shiny, something
...
Because there is so much high quality material on shiny and
web app development overall, it would be easy to repeat what's already
out there. That is not
the intention, so this tutorial builds on and creates a narrative for
existing tutorials, rather than trying to 
rebuild the wheel. The aim is simple: **to navigate new users of R through the exciting new world of online visualisation that it opens up via Shiny**. Before
we proceed I'd like you to take a quick peak at the following shiny resources,
...
In addition to checking out the above resources, aspiring 'web app' creators
should think about what **shiny** is well suited for before diving in.
...
- Map serving: this may be best left to something heavier duty like [GeoServer](http://geoserver.org/)
- Collecting user information (at present)

To see what **shiny** does well and not-so-well, it's worth taking a look at
some examples.

- An excellent real world example is the [Ebola-Dynamic-Model](https://econometricsbysimulation.shinyapps.io/Ebola-Dynamic-Model/) which provides an interactive model of the spread of Ebola.
- The 'word cloud' example in RStudio's [gallery](http://shiny.rstudio.com/gallery/word-cloud.html) demonstrates that it's not *just* quantitative information that
can be produced.
- The 'google-charts' example shows how **shiny** can integrate with existing web
visualisation tools.
- My favourite example is the [superzip](http://shiny.rstudio.com/gallery/superzip-example.html) also from the gallery. This demonstrates the ability to place data on-top of interactive maps.
- There is an academic paper explaining how shiny was used to create a proof-of-concept
app for precision farming [@Jahanshiri2014].
...
- [d3]() is an extremely flexible JavaScript visualisation package, which interfaces to R via the [d3Network](http://cran.r-project.org/web/packages/d3Network/index.html) package.
...
app's name. That's fundamental to the app's structure:
...
with `shinyapps.R` being the only file in the route directory, which can
be used to run the apps in the sub-folders `hi` and `rentSplit`. These
are apps, containing the critical **ui** and **server** components. Note that
`rentSplit` contains another sub-directory `shinyapps`. This repository does not
affect the overall performance of the app: it's used to embed the app online
on the free app hosting site [shinyapps.io](http://www.shinyapps.io/).
...
which colour they want the histogram to be. The end result should look like this:

![modified](/home/robin/repos/robinlovelace.github.io/img/shiny-modified-app.png)

How do we create this? 
First I'd suggest creating a new folder, perhaps called `himod` to contain
the new app's code so it runs from the project's root directory.
Next we must identify some key features of the shiny
app.
